...
def simplify_polygon(geom: Union[Polygon, MultiPolygon], relative_tolerance: float, log: logging.Logger, to_meter_proj: Optional[callable] = None) -> Union[Polygon, MultiPolygon]:
    ...
    if isinstance(geom, MultiPolygon):
        ...
        for i, poly in enumerate(geom.geoms):
            ...
            simplified_poly = simplify_polygon(poly, relative_tolerance, log, to_meter_proj)
    ...
    try:
        ...
        try:
            ...
            for i, (x, y) in enumerate(geom.exterior.coords):
                try:
                    ...
                    log.debug(f"  Error converting exterior coordinate {i}: (x={x}, y={y}), Error: {str(e)}")
                    return geom
            
            # log.debug(f"  Processed {len(exterior_coords)} exterior coordinates")
            exterior_coords = np.array(exterior_coords, dtype=np.float64)
            simplified_exterior = shapely.LineString(exterior_coords).simplify(abs_tolerance, preserve_topology=True)
        ...
        if simplified_exterior.is_valid and not simplified_exterior.is_empty:
                ...
                try:
                    ...
                    for i, (x, y) in enumerate(interior.coords):
                        try:
                            ...
                            log.debug(f"    Error converting interior {ring_idx} coordinate {i}: (x={x}, y={y}), Error: {str(e)}")
                            continue
                    ...
                    if interior_coords:
                        ...
                        simplified_interior = shapely.LineString(interior_coords).simplify(abs_tolerance, preserve_topology=True)
                        if simplified_interior.is_valid and not simplified_interior.is_empty:
            ...
            try:
                ...
                new_poly = Polygon(simplified_exterior, holes=[interior for interior in simplified_interiors])
                if new_poly.is_valid:
                    # Transform back if we transformed to meters
                    if to_meter_proj:
                        try:
                            new_poly = transform(lambda x, y: (x, y), new_poly)  # Transform back to original CRS
...
def simplify_and_convert_to_csv(
    ...
    try:
                ...
                try:
                    ...
                    if not src_proj.is_projected:
                        ...
                        project = pyproj.Transformer.from_crs(
                            src_proj, utm_crs, always_xy=True).transform
                        to_meter_proj = project
                        log.info(f"Setting up transformation to UTM zone {utm_zone} for meter-based simplification")
        ...
        log.info("Using relative tolerance of {:.4f}% of geometry size".format(float(tolerance) * 100))
        ...
        for i, feat in enumerate(features):
            try:
                ...
                try:
                    ...
                    if verbosity >= 2:
                        log.debug(f"  Geometry type: {original_geom.geom_type}  Is valid: {original_geom.is_valid}  Is empty: {original_geom.is_empty}")
                except Exception as e:
                    log.warning(f"Could not create Shapely geometry for feature {i}: {str(e)}")
                    continue
                ...
                try:
                    ...
                    log.warning(f"Could not convert geometry to WKT for feature {i}: {str(e)}")
                    continue

                # Handle polygon geometries specially
                if isinstance(original_geom, (Polygon, MultiPolygon)):
                    simplified = simplify_polygon(original_geom, tolerance, log, to_meter_proj)
                else:
                    ...
                    try:
                        ...
                        for i, (x, y) in enumerate(original_geom.coords):
                            try:
                                ...
                                log.debug(f"    Error converting coordinate {i}: (x={x}, y={y}), Error: {str(e)}")
                                continue
                        ...
                        if coords:
                            ...
                            simplified = original_geom.simplify(abs_tolerance, preserve_topology=True)
                            if to_meter_proj:
                            ...
                            if verbosity >= 2:
                                log.info("  No valid coordinates found, using original geometry")
                ...
                try:
                    ...
                    error_count += 1
                    log.warning(f"Error converting simplified geometry to WKT for feature {i}: {str(e)}")
        ...
        if error_count > 0:
            log.warning(f"Failed to process {error_count} out of {len(features)} features")
        
        if not simplified_geoms:
            return False, "No features could be processed"
        
        avg_reduction = total_reduction / len(simplified_geoms) if simplified_geoms else 0
        log.info("Average vertex reduction across all features: {:.1f}%".format(float(avg_reduction)))

...
class TestValidation:
    ...
    def test_validate_input_raises_if_metadata_missing(self):
        ...
        with pytest.raises(util.JobError):
            jobs.validate_input({"foo": {}, "api_key": "my-key"})

    def test_validate_input_raises_if_res_id_missing(self):

        with pytest.raises(util.JobError):
            jobs.validate_input(
                {"metadata": {"ckan_url": "http://www.ckan.org"}, "api_key": "my-key"}
            )
    ...
    def test_validate_input_raises_if_ckan_url_missing(self):
        ...
        with pytest.raises(util.JobError):
            jobs.validate_input(
                {"metadata": {"resource_id": "h32jk4h34k5"}, "api_key": "my-key"}
            )
...
class TestCkanActionCalls:
    ...
    def test_get_resource(self):
        ...
        httpretty.register_uri(
            httpretty.POST,
            url,
            body=json.dumps({"success": True, "result": {"foo": 42}}),
    ...
    def test_send_resource_to_datastore(self):
        ...
        jobs.send_resource_to_datastore(
            {"id": "an_id"}, [], [], False, "my_key", "http://www.ckan.org/"
        )
...
class TestCheckResponse:
    ...
    def test_text_404_ignore(self):
        ...
        r = requests.get("http://www.ckan.org/")
        jobs.check_response(
            r, "http://www.ckan.org/", "Me", good_status=(200, 201, 404)
        )

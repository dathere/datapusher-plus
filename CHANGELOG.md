...
* Bumped the qsv version installed by `Dockerfile.worker`, `.github/workflows/ci.yml`, and `.github/workflows/main.yml` from `20.0.0` to `20.1.0`. `MINIMUM_QSV_VERSION` stays at `20.0.0` — qsv 20.1.0 has no breaking changes (per the [20.1.0 release notes](https://github.com/dathere/qsv/releases/tag/20.1.0): "pipelines built on 20.0.0 will upgrade in place"), so operators are not forced to upgrade their own `qsv_bin`. User-visible improvement: qsv 20.1.0 bumps `qsv-dateparser` 0.14 → 0.15 which adds recognition for ISO 8601 `T`-separated datetimes without a timezone suffix (e.g. `2024-10-11T14:30:00`) — qsv 20.0.0 misclassified these as `String` during `qsv stats --infer-dates`, which in DP+ surfaced as a date-typed column being demoted to text on certain CSV shapes. The regression-test matrix in `tests/test_issue_173_date_format_inference.py` is updated to reflect the new baseline.
